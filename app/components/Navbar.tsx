  ...
  return (
    <>
      <header className="w-full fixed top-0 left-0 z-[100] bg-white shadow-sm flex flex-col">
        ...
        {/* TIER 1: Utility Bar (Black) */}
        <div className="w-full bg-black text-white h-8 flex items-center justify-between px-6 md:px-12 text-[9px] uppercase tracking-[0.2em] font-bold">
          <p className="hidden md:block">Complimentary White-Glove UK Delivery</p>
          <div className="flex space-x-6 ml-auto">
            <Link href="#" className="hover:text-[#D4AF37] transition-colors">Login / Register</Link>
            <Link href="#" className="hover:text-[#D4AF37] transition-colors">Wishlist</Link>
          </div>
        </div>

        {/* TIER 2: Main Header (Search, Logo, Cart) */}
        <div className="w-full max-w-[1500px] mx-auto px-6 md:px-12 h-20 flex items-center justify-between" ref={cartRef}>
          
          {/* Left: Search Bar */}
          <div className="w-1/3 flex items-center">
            <div className="flex border-b border-gray-300 focus-within:border-black transition-colors pb-1 w-full max-w-[250px]">
              <span className="text-gray-400 mr-2">⚲</span> {/* Simple search icon */}
              <input 
                type="text" 
          ...
          <div className="w-1/3 flex justify-center">
            <Link href="/preview" className="text-2xl font-serif tracking-[0.15em] text-black uppercase whitespace-nowrap">
              Blackwood <span className="text-[#D4AF37]">&</span> Rose
            </Link>
          </div>

          {/* Right: Cart Button & Mini Cart */}
          <div className="w-1/3 flex justify-end relative">
            ...
            {isCartOpen && (
              <div className="absolute top-[100%] right-0 mt-6 w-[340px] bg-white shadow-2xl border border-gray-100 z-[120] flex flex-col animate-fade-in p-6 cursor-default">
        ...
        {/* TIER 3: Category Links with HOVER MEGA-MENUS */}
        <div className="w-full border-t border-gray-200 bg-white h-12 flex items-center justify-center relative">
          <ul className="flex space-x-12 h-full">
            
            {/* LIVING Mega Menu */}
            <li className="group h-full flex items-center">
              <Link href="/preview/shop" className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 group-hover:text-black h-full flex items-center border-b-2 border-transparent group-hover:border-black transition-all">
                Living
              </Link>
              {/* The invisible drop-down panel that appears on hover */}
              <div className="absolute top-full left-0 w-full bg-white shadow-xl border-t border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 py-10 px-12 z-50 flex justify-center gap-24">
                <div>
                ...
                <div>
                  ...
                  <ul className="space-y-3 text-[9px] text-gray-500 tracking-widest uppercase">
                    ...
                    <li><Link href="/preview/shop" className="hover:text-[#D4AF37]">Nest of Tables</Link></li>
                  </ul>
                </div>
                <div>
                  <h4 className="text-[10px] font-bold text-black uppercase tracking-[0.2em] mb-4">Storage</h4>
                  <ul className="space-y-3 text-[9px] text-gray-500 tracking-widest uppercase">
                    <li><Link href="/preview/shop" className="hover:text-[#D4AF37]">Sideboards</Link></li>
                    <li><Link href="/preview/shop" className="hover:text-[#D4AF37]">TV Units</Link></li>
                    <li><Link href="/preview/shop" className="hover:text-[#D4AF37]">Bookcases</Link></li>
                  </ul>
                </div>
                ...
                <div className="w-64 aspect-[4/3] bg-gray-100 overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                    <span className="text-white text-[10px] font-bold tracking-[0.2em] uppercase border border-white px-4 py-2">Shop Living</span>
                  </div>
                </div>
              </div>
            </li>
            ...
            <li className="group h-full flex items-center">
              ...
              <div className="absolute top-full left-0 w-full bg-white shadow-xl border-t border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 py-10 px-12 z-50 flex justify-center gap-24">
                ...
                <div className="w-64 aspect-[4/3] bg-gray-100 overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                    <span className="text-white text-[10px] font-bold tracking-[0.2em] uppercase border border-white px-4 py-2">Shop Dining</span>
                  </div>
                </div>
              </div>
            </li>

            {/* BEDROOM Link (No Mega Menu for this one as an example) */}
            <li className="h-full flex items-center">
              <Link href="/preview/shop" className="text-[10px] uppercase tracking-[0.2em] font-bold text-gray-500 hover:text-black h-full flex items-center border-b-2 border-transparent hover:border-black transition-all">
                Bedroom
              </Link>
            </li>

            <li className="h-full flex items-center">
              <Link href="/preview/shop" className="text-[10px] uppercase tracking-[0.2em] font-bold text-[#D4AF37] hover:text-black h-full flex items-center border-b-2 border-transparent hover:border-black transition-all">
                Sale
              </Link>
            </li>
      ...
      {/* INVISIBLE SPACER: 
          Because the 3-tier header is much taller now (32px + 80px + 48px = 160px), 
          we need a taller invisible box so the website doesn't hide underneath it! 
      */}
